...
//	if (!Features.Backtrack || !local_player || !local_player->is_alive()) {
//		if (!records->empty())
//			records->clear();
//
//		return;
...
//
//		while (records[i].size() > 3 && records[i].size() > static_cast<size_t>(time_to_ticks(static_cast<float>(Features.BacktrackTime) / 1000.f)))
//			records[i].pop_back();
//
...
//
//		auto angle = math.calculate_angle(local_player->get_eye_pos(), head_position, cmd->view_angles);
...
//				continue;
//
//			auto angle = math.calculate_angle(local_player->get_eye_pos(), record->head, cmd->view_angles);
//			auto fov = std::hypotf(angle.x, angle.y);
//			if (fov < best_fov) {
//				best_fov = fov;
//				best_record = i;
